...
It contains rust code that generating the cards. It can then invoke inkscape to generate `generated_card_pngs`, which can be dragged straight into thegamecrafter as mini card assets, and then printed and distributed from there.
...
One hope I had was that I'd be able to call resvg and I'd be able to render pngs in pure rust. Resvg isn't working just now. Maybe in a month or so. For now we'll just call out to inkscape.
...
[install Rust](https://www.rust-lang.org/tools/install), Inkscape and the [Rubik](https://fonts.google.com/specimen/Rubik) font. Run `cargo run`.
